...
var AdminConsole = Backbone.View.extend({
    ...
    template: function () {
        return _.template(" <div class='workspace' style='margin-left: -305px'>" +
            ...
            "<ul class='inner_users'><li class='create_user'>添加用户</li></ul></li>" +
            "<% } %>"+
            "<li><strong>数据源管理</strong></li>" +
            "<ul class='dslist'><strong>数据源</strong>"+
            "<ul class='inner_datasource'><li class='create_datasource'>添加数据源</li></ul></ul>" +
            "<ul class='dslist'><strong>数据模型配置文件</strong>"+
            "<ul class='inner_schema'><li class='create_schema'>添加数据模型配置文件</li></ul></ul>" +
            "<li class='license_container'><strong>许可证</strong>" +
            "<ul><li class='license_info'>信息</li>" +
            "<li class='license_users_list'>用户列表</li></ul></li>"+
            "</ul>" +
            "</ul>" +
            "</div>" +
            "</div>" +
    ...
    caption: function() {
        return '<span class="i18n">管理控制台</span>';
    },
...
//                    "rename": {name: "Rename", i18n: true },
    ...
    backup_restore_template: _.template("<div><h1>备份</h1><p><a href='/saiku/rest/saiku/admin/backup' class='form_button'>现在备份!</a></p>" +
        "<hr>" +
        "<h1>恢复</h1>" +
    ...
    usertemplate: _.template(" <form><div id='accordion'><h3 class='accordion-toggle' >用户细节</h3>" +
        "<div class='accordion-content default'>"+
        "<label for='username'>用户名:</label> <input class='form-control'  onfocus=\"this.value=''; this.onfocus=null;\" type='text' name='username' value='<% if(user.username) { %><%= user.username %><%} else{ %>输入用户名<%}%>'><br/>" +
        "<label for='email'>电子邮件地址:</label> <input class='form-control' type='text' onfocus=\"this.value=''; this.onfocus=null;\" name='email' value='<% if(user.email) { %><%= user.email %><%} else{ %>输入电子邮件地址<%}%>'><br/>" +
        "<div class='clear'></div>" +
        "</div>" +
        ...
        "<label for='newpassword'>修改密码:</label><input class='form-control' type='password' value='' name='newpassword'><br/>" +
        "<label for='newpassword2'>重复密码:</label><input class='form-control' type='password' value='' name='newpassword2'><br/>" +
        ...
        "<div class='accordion-content'>"+
        "<label for='roleselect'>已存在的角色: </label><select name='roleselect' class='form-control role_select'" +
        " multiple>" +
        " <% _.each(user.roles, function(role){%><option><%= role %></option><%});%></select><br/><br/>" +
        "<a href='<%= user.id%>' class='remove_role form_button btn btn-default  user_button'>移除被选择的角色</a><br/> " +
        "<a href='#' class='new_remove_role btn btn-default  form_button user_button hide'>移除被选择的角色</a><br/><br/> <br/> " +
        "<label for='role'>添加新的角色: </label><input class='form-control' type='text' name='role'><br/>" +
        ...
        "<br/><br/><a href='<%= user.id%>' class='remove_user btn btn-danger form_button user_button hide'>移除用户</a> " +
        ...
        "</div></div></form>"),
    datasourcetemplate: _.template("<form><h3>创建数据源</h3>"+
        "<div class='simpleConnection'><label for='connname'>名称:</label><input type='text' class='form-control' name='connname' placeholder='数据源名称' value='<%= conn.connectionname %>'/><br />" +
        "<label for='drivertype'>数据处理驱动类型:</label><select name='drivertype' class='form-control drivertype'><option value='MONDRIAN'>Mondrian</option><option value='XMLA'>XMLA</option></select><br/>" +
        "<% if(!Settings.EXT_DATASOURCE_PROPERTIES) { %>"+
        "<label for='jdbcurl'>URL:</label><input name='jdbcurl' class='form-control' placeholder='数据源的路径地址' value='<%= conn.jdbcurl %>' type='text'/><br class='horridbr'/>" +
        "<% } else {	%>"+
        "<input name='jdbcurl' type='hidden'/>" +
        "<% } %>"+
        "<label for='schemapath'>数据模型配置文件:</label><select class='form-control schemaselect' name='schemapath'>" +
        ...
        "<%});%></select><br/>" +
        "<% if(!Settings.EXT_DATASOURCE_PROPERTIES) { %>"+
        "<label for='driver'>Jdbc 驱动: </label><input name='driver' class='form-control' placeholder='数据源连接驱动' value='<%= conn.driver %>' type='text'/><br class='horridbr'/>" +
        "<label for='connusername'>用户名: </label><input name='connusername' class='form-control' type='text' value='<%= conn.username %>'/><br/>" +
        "<label for='connpassword'>密码:</label><input name='connpassword' class='form-control' type='password' value='<%= conn.password %>'/><br/>" +
        "<% } else {	%>"+
        "<input name='driver' type='hidden'/>" +
        "<input name='connusername' type='hidden'/>" +
        "<input name='connpassword' type='hidden'/>" +
        "<% } %>"+
        "<label for='securityselect'>安全类型:</label><select class='form-control securityselect' id='secselect' name='securityselect'>" +
        "<option value='NONE'>无</option><option value='ONE2ONE'>一对一映射</option><option value='PASSTHROUGH'>直通 (for XMLA)</option></select><br/>" +
        "<% if(Settings.EXT_DATASOURCE_PROPERTIES) { %>"+
        "<label for='extpropselect'>外部属性键:</label>" +
        "<select name='extpropselect' class='form-control extpropselect'><option></option>" +
        "<% _.each(properties, function(path){%>" +
        "<option><%= path %></option>"+
        "<%});%>"+
        "</select><% } %><br/></div>" +
        "<div class='advconnection' style='display:none;'><textarea name='adv_text' class='form-control' placeholder='通过编写字符串实现数据源的添加，例如：\n type=OLAP\n name=foodmart \n driver=modrian.olap4j.MondrianOlap4jDriver\n location=jdbc:mondrianL:Jdbc=jdbc:mysql://localhost:3306/foodmart;Catalog=res:da\n username=dbuser\n password=password。' rows='10' cols='75'><%= conn.advanced %></textarea></div>" +
        "<br/><br/><a href='' name='advancedurl' class='advancedurl btn btn-default'>高级</a><% if(Settings.DATA_SOURCES_LOOKUP) { %> " +
        "<a href='' name='getdatasources' class='btn btn-default getdatasources'>数据源</a> <% } %>" +
        "<a href='<%= conn.id%>' class='user_button btn btn-danger form_button remove_datasource hide'>移除</a>" +
        "<a href='<%= conn.id%>' class='user_button form_button btn btn-default save_datasource'>保存</a>" +
        "<a href='<%= conn.id%>' class='refresh_button form_button user_button btn btn-default hide'>刷新" +
        "缓存</a><div class='clear'></div></form>" +
        "<div id='savestatus'></div>"
    ),
    schemauploadtemplate: _.template( "<h3>数据模型配置文件（.xml文件）</h3>" +
        "<input name='fileschema' type='file' class='form-control upload_button'/><div class='clear'></div><br/>" +
        "<label for='schemaname'>文件名称:</label><input name='schemaname' type='text' class='form-control' value='<%=" +
        " schema.id %>'/><br/>" +
        "<a href='<%= schema.id%>' class='user_button form_button btn btn-default remove_schema hide'>移除</a>" +
        "<a href='/saiku/rest/saiku/admin/schema/<%= schema.id%>' class='user_button btn btn-default form_button download_schema hide'>下载</a><input type='submit' class='user_button form-control form_button upload_button submitdatasource' value='上传'>" +
        "<br/><div id='uploadstatus'></div>"),
    licenseInfoTemplate: _.template("<h3>许可证信息</h3>" +
        "<ul class='license_type'><li><strong>许可证类型: </strong></li>" +
        "<li><strong>许可证到期: </strong></li>" +
        "<li><strong>许可证联系人: </strong></li>" +
        "<li><strong>许可证主机名: </strong></li>" +
        "<li><strong>用户限制: </strong></li></ul>"),
    licenseAddUserTemplate: _.template("<form>" +
        "<h3>添加用户</h3><br>" +
        "<label for='username'>用户名:</label> <input class='form-control' type='text' name='username'>" +
        "<a href='#' class='add_license_user btn btn-default form_button user_button'>添加用户</a><div class='clear'></div><br>" +
        "<h3>用户列表</h3>" +
    ...
    save_user: function (event) {
        ...
        if(username===null){
            $.notify('没有用户名不能更新用户', {globalPosition: 'top center', className: 'error'})
        }
        else {
            ...
            user.save({}, {
                ...
                , success: function (e) {
                    $.notify('用户更新成功', {globalPosition: 'top center', className: 'success'});
    ...
    save_password: function (event) {
        ...
        if ($newtarget.val() == $newtarget2.val()) {
            ...
            user.save({}, {data: JSON.stringify(user.attributes), contentType: "application/json", success: function(e){
                $.notify('密码更新成功', { globalPosition: 'top center', className: 'success' });
                $newtarget.val("");
                $newtarget2.val("");
            }});
        }
            ...
            $.notify('错误验证', { globalPosition: 'top center', className: 'error' });
    ...
    $(this.el).find('.inner_users').append("<li class='create_user'>添加用户</li>");
    },
    ...
    uploadFile: function (event) {
        ...
        this.schemas.create({file: file, name: $(this.el).find("input[name='schemaname']").val()}, {processData: true, success: function(){
            $(that.el).find('#uploadstatus').html("上传成功！");
            that.schemas.fetch();
        },error: function(data, xhr){
            $(that.el).find('#uploadstatus').html("上传失败！<br/>("+xhr.responseText+")");
            that.schemas.fetch();
        }});
    },
...
Saiku.events.bind('admin:loaddatasources', function(admin){
    $(admin.admin.el).find('.inner_datasource').append("<li class='i18n create_datasource'>添加数据源</li>");

});
Saiku.events.bind('admin:loadschema', function(admin){
    $(admin.admin.el).find('.inner_schema').append("<li class='i18n create_schema'>添加数据模型配置文件</li>");
